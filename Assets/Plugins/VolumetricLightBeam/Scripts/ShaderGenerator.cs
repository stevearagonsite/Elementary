...
namespace VLB
{
    public class ShaderGenerator : ScriptableObject
    {
        ...
        public class GenPass
        {
            ...
            static void AppendMultiCompile(ref string str, params string[] options)
            {
#if UNITY_2019_1_OR_NEWER
                const string kPrefix = "                #pragma multi_compile_local __";
#else
                const string kPrefix = "                #pragma multi_compile __";
#endif
                str += kPrefix;
            ...
            public string Generate(SRPHelper.RenderPipeline rp, RenderingMode rm, EnabledFeatures enabledFeatures, int passID, int passCount)
            {
                ...
                AppendMultiCompile(ref multiCompileVariants, ShaderKeywords.AlphaAsBlack);
                if (enabledFeatures.noise3D)        AppendMultiCompile(ref multiCompileVariants, ShaderKeywords.Noise3D);
                if (enabledFeatures.depthBlend)     AppendMultiCompile(ref multiCompileVariants, ShaderKeywords.DepthBlend);
                switch(enabledFeatures.colorGradient)
                {
                    case FeatureEnabledColorGradient.HighOnly:      AppendMultiCompile(ref multiCompileVariants, ShaderKeywords.ColorGradientMatrixHigh); break;
                    case FeatureEnabledColorGradient.HighAndLow:    AppendMultiCompile(ref multiCompileVariants, ShaderKeywords.ColorGradientMatrixHigh, ShaderKeywords.ColorGradientMatrixLow); break;
                }
                if (enabledFeatures.dynamicOcclusion)   AppendMultiCompile(ref multiCompileVariants, ShaderKeywords.OcclusionClippingPlane, ShaderKeywords.OcclusionDepthTexture);
                if (enabledFeatures.meshSkewing)        AppendMultiCompile(ref multiCompileVariants, ShaderKeywords.MeshSkewing);
                if (enabledFeatures.shaderAccuracyHigh) AppendMultiCompile(ref multiCompileVariants, ShaderKeywords.ShaderAccuracyHigh);
