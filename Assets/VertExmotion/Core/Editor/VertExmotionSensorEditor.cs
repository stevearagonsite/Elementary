	...
	*/
		...
		public static void DrawSensorSettings( VertExmotionSensorBase sensor )
		{
            ...
            if (GUI.changed)
            {
                ...
                Undo.RecordObject(sensor, "sensor modification");
            }
            ...
            if (m_settingMode == eSettingsMode.SENSORS || m_settingMode == eSettingsMode.FX )
            {
                ...
                if (VertExmotionBase.GetScaleFactor(sensor.transform) > 0)
                {
                    sensor.m_envelopRadius =
                        Vector3.Distance(
                                             Handles.FreeMoveHandle(sensor.transform.position + svCam.transform.right * sensor.m_envelopRadius * VertExmotionBase.GetScaleFactor(sensor.transform), Quaternion.identity, (constUnit * .02f), Vector3.zero, CubeCap)
			...
			if( m_settingMode == eSettingsMode.COLLIDERS )
			{
				...
				if( vtmCol != null)
				{
					for(int i=0; i<vtmCol.m_collisionZones.Count;++i)
					{
						...
						vtmCol.m_collisionZones[i].positionOffset = vtmCol.transform.InverseTransformPoint( Handles.FreeMoveHandle( worldColZonePos, Quaternion.identity, ( constUnit*.02f ), Vector3.zero, CircleCap ) );
					}
				}
			}
