...
float4 VertExmotionBase(float4 wrldPos, float4 col, inout float weight)
{
	...
	for (int i = 0; i<NB_SENSORS; ++i)
	{
		...
		if (dist < KVM_RadiusCentripetalTorque[i].x)
		{
			...
			float p = pow(abs(f), KVM_MotionDirection[i].w);
			weight = i == 0 ? f : (weight + f)*.5;
			f = p;
			if (length(KVM_MotionAxis[i].xyz)>0)
				torqueDir = mul(RotAxis(KVM_MotionAxis[i].xyz, KVM_RadiusCentripetalTorque[i].z * PI_180 * f), (wrldPos - KVM_SensorPosition[i]).xyz) - (wrldPos - KVM_SensorPosition[i]).xyz;
...
{
	float w = 0;
	return mul(unity_WorldToObject, VertExmotionBase(float4(wrldXYZ, wrldW), float4(col, 0), w) - float4(wrldXYZ, 0)).xyz;
}
...
#endif
	return mul(unity_WorldToObject, newpos - float4(wrldXYZ, 0)).xyz;
}
