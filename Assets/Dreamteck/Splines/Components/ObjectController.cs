    ...
    public class ObjectController : SplineUser
    {
        ...
        protected override void Build()
        {
            ...
            for (int i = 0; i < spawned.Length; i++)
            {
                ...
                if(_offsetUseWorldCoords) spawned[i].position += _offset;
                else spawned[i].position += -right * _offset.x + evaluateResult.normal * _offset.y;
                if (_applyRotation)
                {
        ...
        {
            base.OnDestroy();
            //if (objectMethod == ObjectMethod.Instantiate) Clear();
        }
    }
}
