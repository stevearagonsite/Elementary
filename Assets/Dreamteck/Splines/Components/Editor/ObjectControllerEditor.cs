...
public class ObjectControllerEditor : SplineUserEditor
{
        ...
        if(user.offsetUseWorldCoords) user.offset = EditorGUILayout.Vector3Field("Offset", user.offset);
        else user.offset = EditorGUILayout.Vector2Field("Offset", user.offset);
        EditorGUI.indentLevel++;
        user.offsetUseWorldCoords = EditorGUILayout.Toggle("Use World Coords.", user.offsetUseWorldCoords);
        EditorGUI.indentLevel--;
